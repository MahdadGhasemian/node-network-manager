...
const clib = args => new Promise((resolve, reject) => {
  let resolved = false;
  try {
    const nmcli = spawn("nmcli",args);
    const body = [];
...
const activityMonitor = stream => Promise((resolve, reject) => {
  try {
...
const setHostName = hostName => cli(['general','hostname', String(hostName)]);
// networking
const enable = () => cli(['networking', 'on']);
const disable = () => cli(['networking', 'off']);
const getNetworkConnectivityState = (reChecking = false) => cli(reChecking ? ["networking", "connectivity", "check"] : ["networking", "connectivity"]);
// connections (profiles)
const connectionUp = profile => cli(['connection','up', String(profile)]);
const connectionDown = profile => cli(['connection','down', String(profile)]);
const getConnectionProfilesList = (active = false) => clib(active ? ["-m", "multiline", "connection", "show", "--active", "--order", "active:name"] : ["-m", "multiline", "connection", "show", "--order", "active:name"]);
// devices
...
const deviceStatus = async () => {
  ...
  return Object.keys(data[0]).map(line => {
    if (line.startsWith('DEVICE')) return null; // filter first line
    const lines = line.replaceAll(/\s{2,}/g,' ').trim().split(' ');
    const ret = {};
...
const wifiHotspot = async (ifname, ssid, password) => clib(['device', 'wifi', 'hotspot', 'ifname', String(ifname), 'ssid', ssid, 'password', password ]);
const wifiCredentials = async ifname => {
  if (!ifname) throw Error('ifname required!');
  const data = await clib(['device', 'wifi', 'show-password', 'ifname', ifname]);
  return data[0];
}
const getWifiList = async (reScan = false) => {
  const data = await clib(reScan ? ["-m", "multiline", "device", "wifi", "list", "--rescan", "yes"] : ["-m", "multiline", "device", "wifi", "list", "--rescan", "no"]);
  return data.map(el => {
...
const wifiConnect = (ssid, password) => cli(['device','wifi','connect', String(ssid),'password', String(password)]);
